...
const apiBaseUrl = "https://assinatura-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Assinante</li>
          <li>✓ Controle completo de Pagamento</li>
          <li>✓ Controle completo de Cancelamento Assinatura</li>
        </ul>
      </div>
...
function PainelPlano({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "descricao", "preco", "ciclo", "recursos", "ativo", "trialDias", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "descricao", "preco", "ciclo", "recursos", "ativo", "trialDias"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/plano" + item.id + "/320/200"} alt="Plano" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            ...
            <div className="item-field"><b>ciclo</b><span>{item["ciclo"] ?? "—"}</span></div>
            <div className="item-field"><b>recursos</b><span>{item["recursos"] ?? "—"}</span></div>
            <div className="item-field"><b>ativo</b><span>{item["ativo"] != null ? (item["ativo"] ? "Sim" : "Não") : "—"}</span></div>
            <div className="item-field"><b>trialDias</b><span>{item["trialDias"] ?? "—"}</span></div>
      ...
      <input type="number" value={form["ciclo"] ?? ""} onChange={e => setForm({...form, ciclo: e.target.value})} />
      <label className="field-label">recursos</label>
      <input type="text" value={form["recursos"] ?? ""} onChange={e => setForm({...form, recursos: e.target.value})} />
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["ativo"] === true || form["ativo"] === "true"} onChange={e => setForm({...form, ativo: e.target.checked})} />
        ativo
      </label>
      <label className="field-label">trialDias</label>
      <input type="number" value={form["trialDias"] ?? ""} onChange={e => setForm({...form, trialDias: e.target.value})} />
              <div className="modal-actions">
...
function PainelAssinante({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "empresa", "planoId", "dataInicio", "dataRenovacao", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/assinantes", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/assinantes/" + editId : apiBaseUrl + "/api/assinantes";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "empresa", "planoId", "dataInicio", "dataRenovacao", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["plano"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/assinantes/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["nome"] ?? "Assinante")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/assinantes/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Assinante")}</div>
            ...
            <div className="item-field"><b>dataRenovacao</b><span>{item["dataRenovacao"] ? new Date(item["dataRenovacao"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Assinante</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      ...
      <label className="field-label">dataRenovacao</label>
      <input type="datetime-local" value={form["dataRenovacao"] ?? ""} onChange={e => setForm({...form, dataRenovacao: e.target.value})} />
...
function PainelPagamento({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const [pagina, setPagina] = React.useState(1);
  const [modoKanban, setModoKanban] = React.useState(false);
  const [assinanteList, setAssinanteList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["assinanteId", "data", "valor", "periodo", "status", "formaPagamento", "codigoTransacao", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/assinantes", { headers: authHeaders(token) }).then(r => r.json()).then(setAssinanteList).catch(() => {});
  }, [token]);
  ...
  const editar = (item) => {
    const f = {};
    ["assinanteId", "data", "valor", "periodo", "status", "formaPagamento", "codigoTransacao"].forEach(k => { f[k] = item[k] ?? ""; });
    ["assinante"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["assinanteId"] ?? "Pagamento")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["assinanteId"] ?? "Pagamento")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            ...
            <div className="item-field"><b>periodo</b><span>{item["periodo"] ?? "—"}</span></div>
            <div className="item-field"><b>formaPagamento</b><span>{item["formaPagamento"] ?? "—"}</span></div>
            <div className="item-field"><b>codigoTransacao</b><span>{item["codigoTransacao"] ?? "—"}</span></div>
            <div className="item-field"><b>assinante</b><span>{item.assinanteId ? ((assinanteList.find(o => o.id === item.assinanteId) || {}).nome ?? ("#" + item.assinanteId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">assinanteId</label>
      <input type="text" value={form["assinanteId"] ?? ""} onChange={e => setForm({...form, assinanteId: e.target.value})} />
      ...
      <label className="field-label">codigoTransacao</label>
      <input type="text" value={form["codigoTransacao"] ?? ""} onChange={e => setForm({...form, codigoTransacao: e.target.value})} />
      <label className="field-label">assinante</label>
      <select value={form["assinante"] ?? ""} onChange={e => setForm({...form, assinante: e.target.value})}>
        <option value="">Selecione...</option>
        {(assinanteList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelCancelamentoAssinatura({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["assinanteId", "data", "motivo", "planoId", "valorReembolso", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/cancelamentoassinaturas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/cancelamentoassinaturas/" + editId : apiBaseUrl + "/api/cancelamentoassinaturas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["assinanteId", "data", "motivo", "planoId", "valorReembolso", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["assinante", "plano"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/cancelamentoassinaturas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["motivo"] ?? "CancelamentoAssinatura")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/cancelamentoassinaturas/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["motivo"] ?? "CancelamentoAssinatura")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>assinanteId</b><span>{item["assinanteId"] ?? "—"}</span></div>
            <div className="item-field"><b>data</b><span>{item["data"] ? new Date(item["data"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>planoId</b><span>{item["planoId"] ?? "—"}</span></div>
            <div className="item-field"><b>valorReembolso</b><span>{item["valorReembolso"] != null ? Number(item["valorReembolso"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>assinante</b><span>{item.assinanteId ? ((assinanteList.find(o => o.id === item.assinanteId) || {}).nome ?? ("#" + item.assinanteId)) : "—"}</span></div>
            <div className="item-field"><b>plano</b><span>{item.planoId ? ((planoList.find(o => o.id === item.planoId) || {}).nome ?? ("#" + item.planoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => gerarPix(item.valorReembolso)}>Cobrar via Pix</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} CancelamentoAssinatura</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">assinanteId</label>
      <input type="number" value={form["assinanteId"] ?? ""} onChange={e => setForm({...form, assinanteId: e.target.value})} />
      <label className="field-label">data</label>
      <input type="datetime-local" value={form["data"] ?? ""} onChange={e => setForm({...form, data: e.target.value})} />
      ...
      <label className="field-label">planoId</label>
      <input type="number" value={form["planoId"] ?? ""} onChange={e => setForm({...form, planoId: e.target.value})} />
      <label className="field-label">valorReembolso</label>
      <input type="number" step="0.01" value={form["valorReembolso"] ?? ""} onChange={e => setForm({...form, valorReembolso: e.target.value})} />
      <label className="field-label">status</label>
      <select value={form["status"] ?? "ATIVO"} onChange={e => setForm({...form, status: e.target.value})}>
        <option value="ATIVO">ATIVO</option>
        <option value="PENDENTE">PENDENTE</option>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Assinatura<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Plano" ? " active" : "")} onClick={() => setAba("Plano")}><span className="nav-ico">🗂️</span>Plano</button>
        <button className={"nav-btn" + (aba === "Assinante" ? " active" : "")} onClick={() => setAba("Assinante")}><span className="nav-ico">🗂️</span>Assinante</button>
        <button className={"nav-btn" + (aba === "Pagamento" ? " active" : "")} onClick={() => setAba("Pagamento")}><span className="nav-ico">💳</span>Pagamento</button>
        <button className={"nav-btn" + (aba === "CancelamentoAssinatura" ? " active" : "")} onClick={() => setAba("CancelamentoAssinatura")}><span className="nav-ico">🗂️</span>CancelamentoAssinatura</button>
      ...
      {aba === "Assinante" && <PainelAssinante token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Pagamento" && <PainelPagamento token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "CancelamentoAssinatura" && <PainelCancelamentoAssinatura token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
